#!/usr/bin/env python3
"""
Rebuild Vector Database with Improved Data

Uses the improved chunks to rebuild ChromaDB.
"""
...
def rebuild_database():
    """Rebuild ChromaDB with improved chunks."""
    print(f"\n{'='*60}")
    ...
    master_file = embeddings_dir / "all_legal_embedded.json"
    
    if not master_file.exists():
        print("✗ No embedded chunks found!")
        print("Run: python src/rag/embedder.py first")
        return False
    
    print(f"Loading chunks from: {master_file.name}")
    ...
    if duplicates_skipped > 0:
        print(f"⚠️  Fixed {duplicates_skipped} duplicate IDs")
